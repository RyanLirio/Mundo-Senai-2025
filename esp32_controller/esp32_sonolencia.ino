...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    switch (comando) {
      case '0':  // Estado normal - LED desligado
        pararPiscar();
        Serial.println("Estado: Normal");
        break;
        
      case '1':  // Estado de alerta - LED aceso fixo
      ...
      case '2':  // Estado de perigo - LED piscando rápido
        iniciarPiscar();
        Serial.println("Estado: Perigo - LED Piscando!");
        break;
    }
  }

  // Controle do LED piscante no estado de perigo
  if (piscando && estado_atual == ESTADO_PERIGO) {
